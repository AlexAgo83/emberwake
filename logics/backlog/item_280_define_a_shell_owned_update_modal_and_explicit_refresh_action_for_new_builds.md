...
```mermaid
%% logics-kind: backlog
%% logics-signature: backlog|define-a-shell-owned-update-modal-and-ex|req-074-define-a-pwa-update-prompt-and-s|detect-when-a-newer-deployed-build|ac1-the-request-defines-a-player-facing
flowchart LR
    Request[req_074_define_a_pwa_update_prompt_and_sel] --> Problem[Detect when a newer deployed build]
    Problem --> Scope[Define a shell owned update modal]

...
- Once the panel opens, the user lands directly in a flat list of actions without a strong contextual header establishing whether the runtime is live, paused, in settings, or recovering from an issue.

# Scope
- In: Defining the stateful trigger posture, the opened-menu context header, status wording, and how runtime or shell state should surface visually before the action list.
...
flowchart LR
    ...
    Gap --> Slice[Define stateful trigger and context header]
...
- AC2: The slice defines a contextual header or equivalent status band inside the opened menu before the action groups.
- AC3: The slice defines how states such as live runtime, paused runtime, settings scene, and recovery or retry posture should map to trigger and header messaging.
...
- AC2 -> Scope: Entry context is explicit. Proof target: header spec or rendered component structure.

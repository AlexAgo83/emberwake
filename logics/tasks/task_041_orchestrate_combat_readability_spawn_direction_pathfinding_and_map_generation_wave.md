...
# Plan
- [ ] 1. Define and implement overhead health bars, attack-charge bars, and stacked layout rules for combatants.
- [ ] 2. Define and implement forward-biased hostile spawning with bounded side/rear fallback and neutral low-motion behavior.
- [ ] 3. Define and implement first damage-reaction FX and floating damage numbers with upward fade/cleanup behavior.
- [ ] 4. Define and implement a low-cost hostile pathfinding slice with bounded search, refresh rules, and pursuit fallback integration.
- [ ] 5. Define and implement softer world generation with reduced obstacle/modifier density and blob-like clustering.
- [ ] 6. Validate the resulting runtime end to end so readability, navigation, and traversal remain coherent and performant.
- [ ] 7. Update linked requests, backlog, task, and supporting notes so the wave remains traceable.
...
- [ ] Combatants expose readable overhead bars and hit feedback in runtime.
- [ ] Hostile spawns bias forward when the player is moving without breaking spawn safety.
- [ ] Hostiles route around blocked spaces through a bounded low-cost pathfinding posture.
- [ ] Map generation is softer and more clustered while remaining deterministic and playable.
